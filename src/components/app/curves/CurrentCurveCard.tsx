...
import { UpgradePromptCard } from '@/components/app/feedback/UpgradePromptCard';
import { ExpiredOverlay } from '@/components/app/feedback/ExpiredOverlay';
...
export function CurrentCurveCard({
  ...
  const { isExpired, isAtDailyLimit, tier, features } = useSubscription();
  
  // Determine slider step based on tier (0.5 for Basic, 0.1 for Plus/Ultra)
  const feedbackStep = features?.feedback_increment ?? 0.5;
  ...
  const slidersDisabled = isSubmittingFeedback || isExpired || isAtDailyLimit || !dailyLimit || dailyLimit.remaining === 0;
  
  const canSubmitFeedback = onSubmitFeedback && dailyLimit && dailyLimit.remaining > 0 && !isExpired && !isAtDailyLimit;
  ...
  // Check if we should show the upgrade prompt instead of sliders
  const showUpgradePrompt = !isExpired && isAtDailyLimit;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2">
        ...
        {onSubmitFeedback && (
          <div className="p-6 space-y-6 relative">
            {/* Expired overlay - covers entire feedback section */}
            ...
            {/* Show upgrade prompt when daily limit is exhausted, otherwise show sliders */}
            {showUpgradePrompt ? (
              <UpgradePromptCard 
                currentTier={tier} 
                adjustmentsUsed={features?.daily_adjustments_used ?? 0} 
              />
            ) : (
              <>
                {/* Scale legend */}
                ...
                {/* Sliders with hints - now with tier-based step */}
